...
export default function CommentMes() {
  ...
  const getData = () => {
    get(api.message.myPage, { type: "COMMENT", page, size }).then(res => {
      setData(res);
    });
  };
  ...
  const allRead = () => {
    get(api.message.read, { type: "COMMENT" }).then(res => {
      if (res) {
        getData();
      }
    });
  ...
  const allDelete = () => {
    remove(api.message.delete, { type: "COMMENT" }).then(res => {
      if (res) {
        message.success("删除成功");
        getData();
      }
              ...
              <div className="mesl-top">
                <div className="mesl-left">
                  ...
                  <div className="mes-text">
                    ...
                    》回复你: <span className="mes-comment">{m.content}</span>
                  </div>
                </div>
