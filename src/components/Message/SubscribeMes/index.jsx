...
export default function SubscribeMes() {
  ...
  const getData = () => {
    get(api.message.myPage, { type: "SUBSCRIBE", page, size }).then(res => {
      setData(res);
    });
  };
  ...
  const allRead = () => {
    get(api.message.read, { type: "SUBSCRIBE" }).then(res => {
      if (res) {
        getData();
      }
    });
  ...
  const allDelete = () => {
    remove(api.message.delete, { type: "SUBSCRIBE" }).then(res => {
      if (res) {
        message.success("删除成功");
        getData();
      }
