...
export default function LikeMes() {
  ...
  const getData = () => {
    get(api.message.myPage, { type: "LIKE", page, size }).then(res => {
      setData(res);
    });
  };
  ...
  const allRead = () => {
    get(api.message.read, { type: "LIKE" }).then(res => {
      if (res) {
        getData();
      }
    });
  ...
  const allDelete = () => {
    remove(api.message.delete, { type: "LIKE" }).then(res => {
      if (res) {
        message.success("删除成功");
        getData();
      }
